    ...
    <div class="flex h-screen overflow-hidden">
        <aside class="w-68 sidebar-gradient text-white hidden md:flex md:flex-col flex-shrink-0 z-30 shadow-2xl">
            ...
            <nav class="flex-1 overflow-y-auto px-4 py-6 space-y-1">
                ...
                <div x-data="{ open: {{ request()->routeIs('histori.*', 'po.*', 'invoice.*', 'soa.*') ? 'true' : 'false' }} }" class="mt-4">
                    <button @click="open = !open" onclick="toggleSubmenu('histori-submenu', 'histori-icon')"
                        ...
                        <svg id="histori-icon" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-slate-500 transition-transform duration-300 {{ request()->routeIs('histori.*', 'po.*', 'invoice.*', 'soa.*') ? 'rotate-180' : '' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>

                    <div id="histori-submenu" class="submenu space-y-1 {{ request()->routeIs('histori.*', 'po.*', 'invoice.*', 'soa.*') ? 'open' : '' }}">
                        ...
                        <a href="{{ route('soa.histori') }}" class="flex items-center px-4 py-2 text-sm text-slate-400 hover:text-white hover:translate-x-1 transition-transform italic {{ request()->routeIs('soa.*') ? 'text-white font-semibold' : '' }}">
                            <span class="mr-2 text-orange-500">•</span> Histori SOA
                        </a>
                    </div>
                </div>
            </nav>
