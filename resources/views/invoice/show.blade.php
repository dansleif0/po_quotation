...
<div class="container mx-auto my-6 px-4">
    ...
    {{-- Action Bar --}}
    <div class="max-w-[26cm] mx-auto mb-3 flex flex-wrap justify-end items-center gap-2 print:hidden">
        <a href="{{ route('invoice.histori') }}" class="bg-gray-200 text-gray-700 font-bold py-2 px-4 rounded hover:bg-gray-300 transition-colors">
        ...
        <div class="relative inline-block text-left" id="print-dropdown-container">
            <button type="button" onclick="togglePrintMenu()" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded shadow-md inline-flex items-center gap-2 transition-colors cursor-pointer" id="print-dropdown-btn">
                <span>🖨️</span> Print Dokumen
            ...
            <div id="print-menu" class="hidden absolute right-0 mt-2 w-64 rounded-md shadow-xl bg-white ring-1 ring-black ring-opacity-5 z-50 py-1 divide-y divide-gray-100">
                <a href="{{ route('invoice.print', $invoice->id) }}" target="_blank" class="flex items-center gap-3 px-4 py-2.5 text-xs text-gray-800 font-semibold hover:bg-blue-50 hover:text-blue-600 transition-colors">
                    <span class="text-base">📄</span> Print Invoice (PDF)
                </a>
                <a href="{{ route('invoice.print_surat_jalan', $invoice->id) }}" target="_blank" class="flex items-center gap-3 px-4 py-2.5 text-xs text-gray-800 font-semibold hover:bg-emerald-50 hover:text-emerald-600 transition-colors">
                    <span class="text-base">🚚</span> Print Surat Jalan
                </a>
                <a href="{{ route('invoice.print_both', $invoice->id) }}" target="_blank" class="flex items-center gap-3 px-4 py-2.5 text-xs text-indigo-700 font-bold hover:bg-indigo-50 transition-colors text-left cursor-pointer">
                    <span class="text-base">🖨️</span> Print Keduanya (Invoice & Surat Jalan)
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-[26cm] mx-auto bg-white p-8 md:p-10 shadow-xl rounded-lg border text-black font-sans text-[11px]" id="invoice-print-area">
        ...
        <div class="flex justify-between items-start pb-1 gap-2">
            ...
            <div class="flex items-start gap-3 w-6/12">
                <img src="{{ asset('images/logo-tasniem.png') }}" alt="Logo Tasniem" class="h-14 w-auto object-contain shrink-0">
                <div class="text-[11px] leading-snug text-black">
            ...
            <div class="w-4/12 flex justify-end items-start">
                <img src="{{ asset('images/logo-jotun.png') }}" alt="Jotun Logo" class="h-11 w-auto object-contain">
            </div>
        </div>

        {{-- META INFO SECTION --}}
        <div class="grid grid-cols-2 gap-4 my-2 text-[11px] font-sans">
        ...
        {{-- MAIN TABLE (Tighter vertical spacing py-0.5) --}}
        <div class="my-1.5">
