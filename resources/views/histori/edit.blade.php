...
<div class="container mx-auto px-4 max-w-7xl">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-800 tracking-tight flex items-center gap-3">
                <span class="p-2 bg-amber-500 text-white rounded-xl shadow-md shadow-amber-500/30">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                </span>
                Edit Quotation Penawaran
            </h1>
            <p class="text-slate-500 text-sm mt-1">Formulir pengeditan quotation resmi — No. Surat: <span class="font-mono font-bold text-amber-700">{{ $offer->no_surat }}</span></p>
        </div>
    ...
    {{-- Main Form Container --}}
    <form action="{{ route('histori.update', $offer->id) }}" method="POST" id="quotationForm">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-slate-200/80 p-6 md:p-8 mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" name="no_surat" id="no_surat" value="{{ old('no_surat', $offer->no_surat) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="project_no" id="project_no" value="{{ old('project_no', $offer->project_no) }}" placeholder="Contoh: PRJ-2026-001"
                ...
                <div>
                    ...
                    <input type="text" name="perihal" id="perihal" value="{{ old('perihal', $offer->perihal) }}" placeholder="Subject surat..."
                        class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm text-slate-800">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                        ...
                        <div class="relative flex-1">
                            <select id="client_select" required class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm text-slate-800 appearance-none bg-white pr-10 shadow-sm font-medium">
                                ...
                                <option value="{{ $client->id }}" data-nama="{{ $client->nama_klien }}" data-details="{{ $client->client_details ?? $client->alamat }}"
                                    {{ $client->id == old('client_id', $offer->client_id) || $client->nama_klien == old('nama_klien', $offer->nama_klien) ? 'selected' : '' }}>
                                    {{ $client->nama_klien }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    <input type="hidden" name="client_id" id="client_id" value="{{ old('client_id', $offer->client_id) }}">
                    <input type="hidden" name="nama_klien" id="nama_klien" value="{{ old('nama_klien', $offer->nama_klien) }}">
                </div>
                ...
                <div>
                    ...
                    <textarea name="client_details" id="client_details" rows="3" placeholder="Alamat lengkap, Attn/UP, Telepon, Email..."
                        class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm text-slate-800">{{ old('client_details', $offer->client_details) }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="mt-8 pt-6 border-t border-slate-200/80 flex justify-end">
                <div class="w-full md:w-80 space-y-3 bg-slate-50 p-5 rounded-2xl border border-slate-200/60">
                    ...
                    <div class="flex justify-between items-center text-sm gap-4">
                        ...
                        <input type="number" min="0" name="diskon_global" id="diskon_global" value="{{ old('diskon_global', $offer->diskon_global ?? 0) }}" placeholder="0"
        ...
        {{-- Form Actions --}}
        <div class="flex flex-col sm:flex-row items-center justify-end gap-4 mb-12">
            <a href="{{ route('histori.index') }}"
                style="background-color: #f1f5f9 !important; color: #475569 !important;"
                class="w-full sm:w-auto px-6 py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-sm transition-all text-center cursor-pointer">
                Batal
            </a>
            <button type="submit"
                onclick="document.getElementById('action_input').value = 'save';"
                style="background-color: #1e293b !important; color: #ffffff !important;"
                class="w-full sm:w-auto px-7 py-3.5 bg-slate-800 hover:bg-slate-900 text-white font-bold rounded-xl text-sm transition-all shadow-xl flex items-center justify-center gap-2 cursor-pointer shrink-0">
                ...
                <span class="font-bold text-white tracking-wide">Update (Simpan Perubahan)</span>
            </button>
            <button type="submit"
                ...
                class="w-full sm:w-auto px-7 py-3.5 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-xl text-sm transition-all shadow-xl shadow-amber-500/25 flex items-center justify-center gap-2 cursor-pointer shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 fill-current text-white" viewBox="0 0 20 20">
                    <path d="M7 9a2 2 0 012-2h6a2 2 0 012 2v6a2 2 0 01-2 2H9a2 2 0 01-2-2V9z" />
                    <path d="M5 3a2 2 0 00-2 2v6a2 2 0 002 2V5h8a2 2 0 00-2-2H5z" />
                </svg>
                <span class="font-bold text-white tracking-wide">Update & Copy (Buat Baru)</span>
            </button>
        </div>
    </form>
</div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (window.TomSelect && clientSelect) {
            ...
            let initVal = inputClientId.value || clientSelect.value;
            if (initVal) {
                tomClient.setValue(initVal);
            }
        }
        ...
        function createRow(initialData = null) {
            ...
            // --- POPULATE INITIAL DATA JIKA ADA ---
            if (initialData) {
                let prodObj = null;
                ...
                inpNama.value = initialData.nama_produk || '';
                updatePackingOptions(tr, prodObj, initialData.packing_size);

                if (initialData.qty_order) {
                    inpQty.value = initialData.qty_order;
                }

                if (initialData.status_produk) {
                    let st = initialData.status_produk;
                    let matchedStatus = false;
                    for (let i = 0; i < selStatus.options.length; i++) {
                        if (selStatus.options[i].value === st) {
                    ...
                    if (!matchedStatus) {
                        selStatus.value = 'OTHER';
                        inpStatusOther.classList.remove('hidden');
                        inpStatusOther.value = st;
                    }
                }
        ...
        btnAddRow.addEventListener('click', function() { createRow(); });
        ...
        // Render Data Lama dari Database jika Ada
